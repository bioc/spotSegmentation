"spotgrid" <-
function(chan1, chan2, rows=NULL, cols=NULL, span=NULL, show=FALSE)
{
        ##
        ## determines the grid for a block in a microarray slide
	##
	## rows      - number of spots in a row
        ##            if omitted, the row grid will not be determined
	## cols      - number of spots in a column 
        ##            if omitted, the column grid will not be determined
	## span     - approximate number of pixels between spots in a
        ##            can be a vector of length 2 if rows/columns differ
	##            default computed from chan dimensions and # spots
	## show     - display the results (this will take a while)
	##
spotgridPeaks <-
function(series,span=3,seed=0)
{
##
## modified from the R news peaks function attributed to Brian Ripley
##
                      if (!(span %% 2)) span <- span + 1
                      ...
                      zmaxcol <- apply(embed(series, span)[,span:1], 1, function(x) {if (length(y <- seq(along = x)[x == max(x)]) == 1) y else 0})
                      halfspan <- (span-1)/2
                      c(rep(FALSE,halfspan),zmaxcol == 1 + halfspan)
}
        isum <- colSums(chan1 + chan2)
	gridcomp <- function(isum,nspots,span){
		...
		for(i in 2:nPeaks) {
			...
			if(length(J) > 1) {
				...
				J <- if(any(M)) (iVales[M])[1] else J[1]
			}
			V[i - 1] <- J
		}
		P <- rep(0, nVales - 1)
		for(i in 2:nVales) {
			...
			if(length(J) > 1) {
				...
				J <- if(any(M)) (iPeaks[M])[1] else J[1]
			}
			P[i - 1] <- J
		}
		...
		# determine relative peak intensities
		...
		# locate valley floor between the first two peaks
		j <- 1
		while(TRUE) {
			if(iVales[j] > iPeaks[i + 1] && iVales[j] < iPeaks[
				i + 2])
				break
			j <- j + 1
		}
        ...
        if (show) {
##        cat("\nplotting sum of channels block image...\n")
          plotBlockImage(sqrt(chan1+chan2))
        }

        rowcut <- colcut <- NA
        if (!is.null(rows) && rows > 0) {
  	  if(is.null(span)) {
		span <- floor(nrow(chan1)/rows)
		if(!(span %% 2))
			span <- span - 1
	  }
          rowcut <- gridcomp(rowSums(chan1 + chan2),rows,
                             if(length(span) == 2) span[1] else span)
  	  ...
  	  if(is.null(span)) {
		span <- floor(ncol(chan1)/cols)
		if(!(span %% 2))
			span <- span - 1
	  }
          colcut <- gridcomp(colSums(chan1 + chan2),cols,
                             if(length(span) == 2) span[2] else span)        
